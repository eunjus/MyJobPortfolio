...
<table class="bbs_list" summary="게시글 제목, 작성자, 작성일, 조회수, 게시글 내용 등..">
	...
	<thead>
		<tr>
			<th scope="col" style="display:none">PostID</th>
			<th scope="col">번호</th>
			<th scope="col">제목</th>
		...
		<c:choose>
			<c:when test="${fn:length(list) > 0}">
				<c:forEach items="${list}" var="row" varStatus="status">
					<tr>
						<td style="display:none">${row.post_number}</td>
						<td>${status.count}</td>
						<td class="left">
							<c:if test="${row.secret_yn == 'y'}">
								<img src="resources/images/s-images/icon_secret.gif" alt="비밀글">
								<a href="${pageContext.request.contextPath}/boardView.do?code=qna_secret&no=${row.post_number}&sub=select"> ${row.post_title}</a>
							</c:if>
							<c:if test="${row.secret_yn == null}">
								<a href="${pageContext.request.contextPath}/boardView.do?code=qna&no=${row.post_number}&sub=select"> ${row.post_title}</a>
							</c:if>	
							 </td>						
	...
	<div class="btn_right"><a href="${pageContext.request.contextPath}/community.do?code=qna_write" class="btn orange">글쓰기</a></div>	
	</div><!-- .sub_board -->
</div><!-- .sub_content -->
<!-- ** layout정리190513  ** -->
					<!-- #서브 컨텐츠 끝 -->
