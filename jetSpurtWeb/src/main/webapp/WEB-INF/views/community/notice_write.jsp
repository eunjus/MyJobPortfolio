	  ...
	  */
	    ...
	    */    
	 	$.ajax({
	    	      ...
	    	      success: function (data) {
	    	    	if(JSON.parse(data)['result'] == "OK"){
	    	    		alert("파일업로드 성공");
	    	    		location.href = "${pageContext.request.contextPath}/community.do?code=notice";
	 				} else
		...
		function fn_updateNotice(form){
			...
			form.arrDeleteFiles.value = delete_files;
			//form.action = "<c:url value='/updateNotice.do?no=${no}' />";		
			//form.method = "post";
		    ...
		    */    
		 	$.ajax({
		    	      ...
		    	      success: function (data) {
		    	    	if(JSON.parse(data)['result'] == "OK"){
		    	    		alert("파일업로드 성공");
		    	    		location.href = "${pageContext.request.contextPath}/boardView.do?code=notice&no=${notice.post_number}";
		 				} else
		 				alert("서버내 오류로 처리가 지연되고있습니다. 잠시 후 다시 시도해주세요");
		...
		function fileCheck(e) {
		    ...
		    if(mFiles.length > 0 && content_files == null){
...
<div class="sub_content">
	...
	<form name="frm" id="frm" encType="multipart/form-data">
		<fieldset>
			...
			<input type="hidden" name="no" value="${notice.post_number}" />	
			...
			<table class="bbs_write"  summary="게시글 작성, 제목, 작성자, 내용, 파일첨부 등등..">
				...
				<tbody>
					<tr>
						<th scope="row">제목</th>
						<td><input type="text" name="title" id="title" value="${notice.post_title}" /><label for="title" class="dn">제목</label></td>
					</tr>
					<tr>
						<th scope="row">내용</th>
						<td>							
							<input type="checkbox" name="is_secret" id="is_secret-y" value="y" /><label for="is_secret-y">비밀글로 작성</label>
							<div class="edit-box" style="width:100%;"><textarea name="content" id="contents" style="height:320px; display: none;" title="내용을 입력하세요.">${notice.post}</textarea>							
							</div>
						</td>
					</tr>
					<tr>
			...
			<div class="btn_wrap ta_center">
				<c:choose>
					<c:when test="${notice.post_title eq null}">						
						<button onclick="fn_insertBoard(this.form); return false;"><a href="javascript://" class="btn btn_point">확인</a></button>
					</c:when>
					<c:otherwise>
